...
<br>각각의 클래스 내부에서 show()메소드를 오버라이딩하여 각각의 목적에 맞도록 다시 선언되었다. 메인함수에서 해당하는 클래스의 인스턴스를 만들고 getR()을 통하여 반지름을 입력받고, Cylinder클래스는 getHeight()를 통하여 높이까지 입력 받는다. 각각의 gets()메소드를 통해 원은 넓이와 둘레를, 원기둥은 겉넓이와 부피를, 구는 겉넓이와 부피를 계산한다. 그 다음 각 클래스의 show()메소드를 통하여 계산한 값을 출력한다.<br>
#written by nhw
