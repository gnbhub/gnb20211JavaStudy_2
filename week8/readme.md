...
// MyFrame 클래스는 JFrame 클래스를 상속받고 동시에 ActionListener를 구현한다. 따라서 프레임이 버튼에서 발생하는 이벤트도 처리할 수 있다.
	...
	public void actionPerformed(ActionEvent e){
//EventTest1 클래스 안에 actionPerformed()가 정의되어 있어야 한다.
		counter++;
...
매개 변수 e는 사용자가 입력한 키보드의 가상 키 번호이다.
 ...
 - 키보드 포커스
 키 이벤트를 받을 수 있는 조건
키 이벤트(Key Event)가 발생하려면 컴포넌트가 반드시 키보드 포커스를 가지고 있어야 한다.
키보드 포커스를 얻으려면 requestFocus(), setFocusable(true)를 호출하여야 한다.
panel.requestFocus();
panel.setFocusable(true);

 - 마우스 이벤트(Mouse Event)
사용자가 마우스 버튼을 누르거나 마우스를 움직일 때 발생한다. 사용자가 버튼을 누르거나 메뉴를 선택할 때는 마우스 이벤트를 처리할 필요가 없다(액션 이벤트로 처리하면 되기 때문).
하지만 사용자가 화면에 마우스로 그림을 그리게 하려면 마우스 이벤트를 처리하여야 한다.
<MouseListener 인터페이스의 추상 메소드>
...
class MyListener implements KeyListener{
 ...
 앞서 말한 리스너의 불편을 해소하기 위한 것. 추상 메소드들 중 필요한 것만 구현하면 된다. 추상메소드가 하나 뿐인 리스너는 어댑터 클래스가 존재하지 않는다.
<어댑터를 사용하는 방법>
class MyListener extends KeyAdapter{  //인터페이스를 구현해 놓은 어댑터 클래스	public void keyTyped(KeyEvent e){	//필요한 메소드만 재정의한다.
 ...
 - 스트림(stream)
 “바이트들의 연속적인 흐름”
우리는 스트림 객체에서 바이트를 읽을 수 있고 스트림 객체에 바이트를 쓸 수도 있다. 스트림은 입출력의 단위에 따라서 분류할 수 있다.
...
public class FileWriterEx{
 ...
 • FileOutputStream
	• FileOutputStream 생성자
		...
		– 현재 내용을 유지하면서, 파일에 새로운 내용을 추가하려 append 인수를 			true로 설정
