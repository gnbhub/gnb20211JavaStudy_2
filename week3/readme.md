...
메인함수{
  ...
  ```
이 예문은 객체지향프로그래밍 관점에서 볼 때는 적절하지 않다. 인스턴스의 필드를 직접 접근한다는 것은 외부에서도 필드에 직접 접근이 가능하다는 것인데, <br>
이렇게 되면 '캡슐화'와는 거리가 먼 코드이다. 적절한 방법은 클래스 내의 선언된 메소드를 이용하여 인스턴스의 필드를 설정하는 것이 바람직하다.
## 연습문제
