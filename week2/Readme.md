...
switch (변수)
 { case c1:
   ...
   case c2:
    ...
    break;}
```
 이런 형태로 작성되는데, 변수가 c1인 경우, 처리문장 1을 수행하고 변수가 c2인 경우, 처리문장 2를 수행한다. <b>break;
 가 없으면 계속해서 아랫줄을 읽어가므로 각 case가 실행되고 switch문이 중단될 수 있도록 <b>break; 가 필요하다. 
...
```
 ...
 ## break문
 하나의 반복문을 벗어날 때 사용한다. break문을 사용하면 현재의 반복문에서 벗어나게 된다. 만약 중첩된 반복문 전체를 종료하고자 한다면 레이블을 이용하고 <b>break 레이블명;
 을 통해 레이블을 종료할 수 있다.
 ## continue문
